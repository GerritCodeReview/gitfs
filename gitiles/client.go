...
const (
	...
	// supported, but googlesource.com doesn't, apparently. In
	// addition, JGit provides ZipFormat, but gitiles doesn't
	// support it.
)
...
// is visible to the caller. Currently, only the 'contains' option is
// implemented, so it must always be specified
