...
func clearLinks(dir, mount string) (string, error) {
...
func diffFiles(oldDir, newDir string) ([]string, error) {
...
func populateCheckout(ro, rw string) error {
	...
	changed, err := diffFiles(oldRoot, ro)
	if err != nil {
		return fmt.Errorf("diffFiles: %v", err)
	}

	// TODO(hanwen): parallellize?
	now := time.Now()
