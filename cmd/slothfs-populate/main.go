...
func main() {
	...
	newROWorkspace := flag.String("ro", "", "path to slothfs-multifs mount.")
	mount := flag.String("mount", "", "slothfs mountpoint. Autodetected if empty.")
	sync := flag.Bool("sync", false, "If set, sync checkout to latest manifest version.")
	syncBranch := flag.String("sync_branch", "master", "For sync, use this branch.")
	syncRepo := flag.String("sync_repo", "platform/manifest", "For sync, get manifest from this repository.")
	flag.Parse()
