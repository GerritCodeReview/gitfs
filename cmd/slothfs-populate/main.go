...
func populateCheckout(ro, rw string) error {
	...
	if wsName != "" {
		go func() {
			...
			oldInfos = t.allFiles()
			log.Println("oldRoot", time.Now().Sub(st))
			errs <- err
		}()
	} else {
	...
	go func() {
		...
		rwTree = t
		errs <- err
		log.Println("rwTree", time.Now().Sub(st))
	}()
	go func() {
		st := time.Now()
		...
		roTree = t
		errs <- err
		log.Println("roTree", time.Now().Sub(st))
	}()
	...
	log.Println("createLinks", time.Now().Sub(st))
	...
	log.Println("changedFiles", time.Now().Sub(st))
	...
	log.Println("chtime", time.Now().Sub(st))
